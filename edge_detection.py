...
img = open(f"C:\\Users\\Hp\\Downloads\\python_images\\person1.BMP", "r+b")
img.seek(18, 0)
...
for r in range(height):
    for c in range(width):
        if (r == 0 or r == height - 1) or (c == 0 or c == width - 1):
            ...
            blueconv = int(Keepinrange(convolution(blue, kernel, r, c))).to_bytes(
                1, "little"
            )
            greenconv = int(Keepinrange(convolution(green, kernel, r, c))).to_bytes(
                1, "little"
            )
            redconv = int(Keepinrange(convolution(red, kernel, r, c))).to_bytes(
                1, "little"
            )
